...
In case there is online access to the data, such as through clouds, it is possible to 

### Device settings

A time-stamped and versioned standard operating procedure (SOP) document can help ensuring reproducibility of device settings when there are multiple operators settings up devices for studies. A key component is to ensure the same parameters, but also other configuration flags being consistently used for logging.
